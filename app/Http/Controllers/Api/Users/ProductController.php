    ...
    public function index()
    {
        $items = Item::whereHas('batches', function ($q) {
            ...
            ->map(function ($item) {
                ...
                } elseif (str_starts_with($item->main_image, 'http')) {
                    $item->image = $item->main_image;

                } else {
                    $item->image = Storage::disk('s3')->url($item->main_image);
                }
                ...
                $item->images = $item->images->map(function ($img) {
                    ...
                    } elseif (str_starts_with($img->image, 'http')) {
                        $img->image_url = $img->image;

                    } else {
                        $img->image_url = Storage::disk('s3')->url($img->image);
                    }
